    ...
    for collega_no in all_collega_no:
        ...
        if all_profess_detail:
            for profess_detail in all_profess_detail:
                ...
                soup = BeautifulSoup(reader,'html.parser')
                all_course_table = soup.find('table',id='fb2').find_all('td',class_='ncontents')
                ...
                for each_course in all_course_table:
                    ...
                    if (count%17==16):
                        ...
                        conn = pymysql.Connect(
                                               ...
                                               charset='utf8')
                        cursor = conn.cursor()
                        sql = 'insert into plan(profess_id,course_name,total_number,credit,credit_type,course_type) values('+str(profess_id)+',"'+str(course_name)+'","'+str(total_number)+'","'+str(credit)+'","'+str(credit_type)+'","学科专业核心课")'
                        print (sql)
                        cursor.execute(sql)
                        conn.commit()
                        cursor.close()
